...
> Generated 2026-06-04. Data: 4 scenes (figurines, ramen, teatime, waldo_kitchen). Per-frame CSV ([per_prompt.csv](../output/diagnostics/lerf_ovs_per_prompt.csv)). Methodology: [lerf_ovs_failure_analysis_plan.md](../THGS/lerf_ovs_failure_analysis_plan.md).
...
- Oracle mIoU = **0.8079**
- Actual mIoU = **0.6165**
- **Gap = 0.1913** ← matching loss, 다른 논문과 직접 비교 가능한 수치

**Failure 분포 (τ=0.25, per-prompt aggregation):**
- Type A (CLIP matching 실패): **18/67 prompts (26.9%)**
- Type A가 차지하는 전체 IoU 손실: **85.3%**

**Type A 내부:**
- A1 (CLIP feature noise, rank ≥ 11): **9/18 = 50.0%**
- A1의 IoU 손실 비중 (within A): **55.6%**
- A1 정답 SP 평균 CLIP rank: **34.3** (in pools of 745–4136 SPs)
...
1. **#1 (유일 valid architectural attack): A1 — per-SP CLIP feature 생성 단계 수정 (`image_encoding.py` + `merge_proj.py`)**
   - 회복 가능 IoU = **7.090** (Type A 손실의 55.6%)
2. A2 / A3 → **❌ inference setting (topk, scoring) 변경 필요 — protocol fix를 깸**

**해석 한 줄:**
> 정답 SP가 NAG 안에 거의 항상 존재 (SAM lift 성공). 그러나 CLIP이 정답 SP를 평균 34등으로 매기는 prompt 9개가 LERF-OVS 헤드룸의 약 47% 를 잠그고 있다.
...
|---|---|---|---|---|---|
| figurines | 0.7942 | 0.6467 | 0.1475 | 0.8246 | 0.7173 |
| ramen | 0.7857 | 0.4742 | 0.3116 | 0.5097 | 0.5649 |
| teatime | 0.9003 | 0.7390 | 0.1614 | 0.7755 | 0.8011 |
| waldo_kitchen | 0.7512 | 0.6062 | 0.1449 | 0.7277 | 0.6618 |
| **ALL (LangSplat mean)** | **0.8079** | **0.6165** | **0.1913** | **0.7094** | **0.6863** |
...
|---|---|---|---|---|---|
| A | 18 | 26.9% | 0.752 | 0.044 | 12.748 |
| B | 1 | 1.5% | 0.073 | 0.073 | 0.000 |
| C | 48 | 71.6% | 0.846 | 0.800 | 2.200 |

- **Type A 잃은 IoU 합 = 12.748** (전체 손실 14.948 중 85.3%)
- **공격 가능 헤드룸 = Type A 18/67 prompts (26.9%)**

### τ = 0.5

| Type | N prompts | Share | Oracle avg | Actual avg | Sum IoU lost |
|---|---|---|---|---|---|
| A | 19 | 28.4% | 0.782 | 0.082 | 13.306 |
| B | 4 | 6.0% | 0.344 | 0.245 | 0.400 |
| C | 43 | 64.2% | 0.873 | 0.835 | 1.631 |
| ? | 1 | 1.5% | 0.437 | 0.825 | -0.388 |

- **Type A 잃은 IoU 합 = 13.306** (전체 손실 14.948 중 89.0%)
- **공격 가능 헤드룸 = Type A 19/67 prompts (28.4%)**
...
|---|---|---|---|---|---|---|---|
| A1 | rank >= 11 | 9 | 50.0% | 0.788 | 0.000 | 34.3 | CLIP feature noisy → multi-view CLIP aggregation / per-SP feature refinement |
| A2 | rank in [2, 10] | 8 | 44.4% | 0.696 | 0.098 | 5.2 | ranking algo limit → matching algo 교체 / canon-contrast 대체 |
| A3 | rank = 1 | 1 | 5.6% | 0.878 | 0.000 | 1.0 | fragmentation / hierarchy → adaptive topk / level sweep |

### Type A1 대표 prompt (rank ≥ 11, oracle 높은 순 top 10)

| Scene | Prompt | N frames | Oracle | Actual | CLIP rank | Pool |
|---|---|---|---|---|---|---|
| waldo_kitchen | cabinet | 1 | 0.980 | 0.000 | 25 | 1282 |
| waldo_kitchen | ottolenghi | 1 | 0.976 | 0.000 | 20 | 1282 |
| figurines | pikachu | 2 | 0.916 | 0.000 | 14 | 640 |
| figurines | bag | 1 | 0.878 | 0.000 | 24 | 640 |
| ramen | onion segments | 7 | 0.803 | 0.000 | 13 | 756 |
| teatime | hooves | 2 | 0.718 | 0.000 | 57 | 3862 |
| waldo_kitchen | spoon | 1 | 0.711 | 0.000 | 42 | 1282 |
| figurines | tesla door handle | 2 | 0.573 | 0.000 | 93 | 640 |
| ramen | hand | 1 | 0.536 | 0.000 | 21 | 756 |
...
|---|---|---|
| Precision (avg) | 0.705 | TP / (TP+FP) per prompt → mean |
| Recall (avg) | 0.663 | TP / (TP+FN) per prompt → mean |
| F1 (avg) | 0.656 | 2·P·R / (P+R) per prompt → mean |
| Accuracy (avg) | 0.9825 | (TP+TN) / total per prompt → mean |
| IoU (avg) | 0.586 | TP / (TP+FP+FN) per prompt → mean |



| Error pattern | Share | Definition |
|---|---|---|
| FP-dominant (FP / (FP+FN) > 0.7) | 32.8% | over-segment |
| FN-dominant (< 0.3) | 47.8% | miss / under-segment |
| Balanced | 19.4% | 양쪽 비슷 |
...
|---|---|---|
| TP (correct foreground) | 3,738,390 | 2.4966% |
| TN (correct background) | 143,558,347 | 95.8722% |
| FP (잘못 칠한 픽셀) | 1,165,828 | 0.7786% |
| FN (놓친 정답 픽셀) | 1,276,781 | 0.8527% |
...
|---|---|---|---|
| Precision | 0.762 | 0.705 | +0.057 |
| Recall | 0.745 | 0.663 | +0.082 |
| F1 | 0.754 | 0.656 | +0.097 |
| Accuracy | 0.9837 | 0.9825 | +0.0012 |
| IoU | 0.605 | 0.586 | +0.019 |


> **주의**: Pixel-pooled Accuracy = 0.9837 가 매우 높아 보이는 건 배경 픽셀이 전체의 ~95.9%를 차지하기 때문 (class imbalance). LERF-OVS 평가에서 accuracy는 보조 지표.
...
|---|---|---|---|---|---|---|---|---|---|---|
| A | 18 | 100,560 | 37,790,966 | 686,689 | 1,055,624 | 0.128 | 0.087 | 0.103 | 0.9560 | 0.055 |
| B | 1 | 646 | 2,142,081 | 2 | 10,695 | 0.997 | 0.057 | 0.108 | 0.9950 | 0.057 |
| C | 48 | 3,637,184 | 103,625,300 | 479,137 | 210,462 | 0.884 | 0.945 | 0.913 | 0.9936 | 0.841 |
...
|---|---|---|---|---|---|---|
| A | 18 | 0.200 | 0.073 | 0.090 | 0.9569 | 0.044 |
| B | 1 | 0.997 | 0.057 | 0.108 | 0.9950 | 0.073 |
| C | 48 | 0.889 | 0.897 | 0.880 | 0.9919 | 0.800 |
...
|---|---|---|---|---|
| A | 18 | 2 (11%) | 2 (11%) | 14 (78%) |
| B | 1 | 0 (0%) | 0 (0%) | 1 (100%) |
| C | 48 | 20 (42%) | 11 (23%) | 17 (35%) |
...
|---|---|---|---|---|---|
| A1 | 9 | 7.090 | 55.6% | ✅ YES (architecture) | per-SP CLIP feature 생성 단계 수정 (image_encoding.py + merge_proj.py). topk/scoring 미변경. |
| A2 | 8 | 4.781 | 37.5% | ❌ NO (inference setting) | rank in [2,10] = topk 키우면 풀리지만 topk=3은 protocol fix. |
| A3 | 1 | 0.878 | 6.9% | ❌ NO (inference setting) | rank=1인데 topk=3가 distractor 포함 → topk=1 축소가 해결이지만 setting 변경. |
...
전체 67 prompts
├── Type C (성공): 48 (71.6%)
└── Type A (실패): 18 (26.9%) — 손실 12.748 (85.3% of total)
    ├── A1 (rank≥11): 9 (50.0% of A) — 손실 7.090 (55.6% of A)
    ├── A2 (rank 2-10): 8
    └── A3 (rank=1): 1
```

→ **A1 9개 prompt만 해결해도 전체 IoU 손실의 약 47% 회복 가능**.

### 5.3 A1의 평균 rank ≈ 34 — CLIP feature가 정답을 못 알아봄

A1의 9개 prompt에서 정답 SP의 CLIP rank가 평균 **34등** (pool ~745-4136 SPs 중). 즉 위쪽 ~33개 SP가 모두 정답보다 CLIP score 높음.
...
|---|---|---|---|---|---|---|
| figurines | 21 | 0.7942 | 0.6467 | 0.1475 | 3 (14%) | 3 (100% of A) |
| ramen | 14 | 0.7857 | 0.4742 | 0.3116 | 7 (50%) | 2 (29% of A) |
| teatime | 14 | 0.9003 | 0.7390 | 0.1614 | 3 (21%) | 1 (33% of A) |
| waldo_kitchen | 18 | 0.7512 | 0.6062 | 0.1449 | 5 (28%) | 3 (60% of A) |
...
|---|---|---|---|---|---|---|---|---|---|---|
| waldo_kitchen | cabinet | 1 | 0.980 | 0.000 | 0.980 | A | 25 | 0.000 | 0.000 | 0.00 |
| waldo_kitchen | ottolenghi | 1 | 0.976 | 0.000 | 0.976 | A | 20 | 0.000 | 0.000 | 0.00 |
| teatime | bear nose | 3 | 0.974 | 0.001 | 0.973 | A | 5 | 0.001 | 0.008 | 0.89 |
| figurines | pikachu | 2 | 0.916 | 0.000 | 0.916 | A | 14 | 0.000 | 0.000 | 0.54 |
| figurines | bag | 1 | 0.878 | 0.000 | 0.878 | A | 24 | 0.000 | 0.000 | 0.00 |
| teatime | coffee mug | 5 | 0.878 | 0.000 | 0.878 | A | 1 | 0.000 | 0.000 | 0.00 |
| ramen | onion segments | 7 | 0.803 | 0.000 | 0.803 | A | 13 | 0.000 | 0.000 | 0.67 |
| ramen | plate | 4 | 0.756 | 0.000 | 0.756 | A | 6 | 0.000 | 0.000 | 0.00 |
| teatime | hooves | 2 | 0.718 | 0.000 | 0.718 | A | 57 | 0.000 | 0.000 | 0.00 |
| waldo_kitchen | spoon | 1 | 0.711 | 0.000 | 0.711 | A | 42 | 0.000 | 0.000 | 0.00 |
| ramen | corn | 5 | 0.711 | 0.004 | 0.706 | A | 2 | 0.016 | 0.003 | 0.16 |
| ramen | sake cup | 6 | 0.719 | 0.042 | 0.677 | A | 10 | 0.066 | 0.209 | 0.79 |
| waldo_kitchen | dark cup | 1 | 0.825 | 0.154 | 0.672 | A | 4 | 0.980 | 0.154 | 0.00 |
| ramen | bowl | 5 | 0.752 | 0.155 | 0.598 | A | 7 | 0.969 | 0.114 | 0.00 |
| figurines | tesla door handle | 2 | 0.573 | 0.000 | 0.573 | A | 93 | 0.000 | 0.000 | 0.25 |


## 8. Deep Analysis (Q1~Q5) — A1 attack 가설 좁히기

Phase 1 결과 (A1이 dominant) 후 attack 후보를 좁히기 위한 5개 sub-질문.
자세한 표는 [lerf_ovs_deep_analysis.md](../THGS/lerf_ovs_deep_analysis.md) 참조.
...
|---|---|---|---|---|
| LangSplat (per-image → per-scene) | 0.8079 | 0.6165 | 0.1913 | 논문/eval_seg.py 표준 |
| Per-prompt mean across frames → per-scene → overall | 0.8110 | 0.5807 | 0.2303 | failure 분석에 사용 |
| Flat prompt mean (all prompts equal weight) | 0.8091 | 0.5860 | 0.2231 | 참고용 (scene size 무시) |
