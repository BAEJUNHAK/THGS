...
- Oracle mIoU = **0.8079**
- Actual mIoU = **0.4340**
- **Gap = 0.3738** ← matching loss, 다른 논문과 직접 비교 가능한 수치

**Failure 분포 (τ=0.25, per-prompt aggregation):**
- Type A (CLIP matching 실패): **24/67 prompts (35.8%)**
- Type A가 차지하는 전체 IoU 손실: **65.5%**

**Type A 내부:**
- A1 (CLIP feature noise, rank ≥ 11): **6/24 = 25.0%**
- A1의 IoU 손실 비중 (within A): **25.6%**
- A1 정답 SP 평균 CLIP rank: **41.8** (in pools of 745–4136 SPs)
...
1. **#1 (유일 valid architectural attack): A1 — per-SP CLIP feature 생성 단계 수정 (`image_encoding.py` + `merge_proj.py`)**
   - 회복 가능 IoU = **4.320** (Type A 손실의 25.6%)
2. A2 / A3 → **❌ inference setting (topk, scoring) 변경 필요 — protocol fix를 깸**

**해석 한 줄:**
> 정답 SP가 NAG 안에 거의 항상 존재 (SAM lift 성공). 그러나 CLIP이 정답 SP를 평균 42등으로 매기는 prompt 6개가 LERF-OVS 헤드룸의 약 17% 를 잠그고 있다.
...
|---|---|---|---|---|---|
| figurines | 0.7942 | 0.3235 | 0.4707 | 0.3477 | 0.8416 |
| ramen | 0.7857 | 0.3146 | 0.4711 | 0.3329 | 0.7794 |
| teatime | 0.9003 | 0.5680 | 0.3323 | 0.5898 | 0.9261 |
| waldo_kitchen | 0.7512 | 0.5300 | 0.2211 | 0.5590 | 0.7871 |
| **ALL (LangSplat mean)** | **0.8079** | **0.4340** | **0.3738** | **0.4574** | **0.8336** |
...
|---|---|---|---|---|---|
| A | 24 | 35.8% | 0.795 | 0.091 | 16.904 |
| B | 1 | 1.5% | 0.073 | 0.010 | 0.063 |
| C | 42 | 62.7% | 0.834 | 0.624 | 8.825 |

- **Type A 잃은 IoU 합 = 16.904** (전체 손실 25.793 중 65.5%)
- **공격 가능 헤드룸 = Type A 24/67 prompts (35.8%)**

### τ = 0.5

| Type | N prompts | Share | Oracle avg | Actual avg | Sum IoU lost |
|---|---|---|---|---|---|
| A | 39 | 58.2% | 0.826 | 0.227 | 23.331 |
| B | 4 | 6.0% | 0.344 | 0.215 | 0.517 |
| C | 23 | 34.3% | 0.878 | 0.778 | 2.291 |
| ? | 1 | 1.5% | 0.437 | 0.784 | -0.347 |

- **Type A 잃은 IoU 합 = 23.331** (전체 손실 25.793 중 90.5%)
- **공격 가능 헤드룸 = Type A 39/67 prompts (58.2%)**
...
|---|---|---|---|---|---|---|---|
| A1 | rank >= 11 | 6 | 25.0% | 0.720 | 0.000 | 41.8 | CLIP feature noisy → multi-view CLIP aggregation / per-SP feature refinement |
| A2 | rank in [2, 10] | 12 | 50.0% | 0.854 | 0.120 | 2.7 | ranking algo limit → matching algo 교체 / canon-contrast 대체 |
| A3 | rank = 1 | 6 | 25.0% | 0.752 | 0.123 | 1.0 | fragmentation / hierarchy → adaptive topk / level sweep |
...
|---|---|---|
| Precision (avg) | 0.444 | TP / (TP+FP) per prompt → mean |
| Recall (avg) | 0.826 | TP / (TP+FN) per prompt → mean |
| F1 (avg) | 0.520 | 2·P·R / (P+R) per prompt → mean |
| Accuracy (avg) | 0.9444 | (TP+TN) / total per prompt → mean |
| IoU (avg) | 0.424 | TP / (TP+FP+FN) per prompt → mean |



| Error pattern | Share | Definition |
|---|---|---|
| FP-dominant (FP / (FP+FN) > 0.7) | 76.1% | over-segment |
| FN-dominant (< 0.3) | 10.4% | miss / under-segment |
| Balanced | 13.4% | 양쪽 비슷 |
...
|---|---|---|
| TP (correct foreground) | 4,549,950 | 3.0386% |
| TN (correct background) | 135,548,421 | 90.5229% |
| FP (잘못 칠한 픽셀) | 9,175,754 | 6.1278% |
| FN (놓친 정답 픽셀) | 465,221 | 0.3107% |
...
|---|---|---|---|
| Precision | 0.331 | 0.444 | -0.113 |
| Recall | 0.907 | 0.826 | +0.081 |
| F1 | 0.486 | 0.520 | -0.034 |
| Accuracy | 0.9356 | 0.9444 | -0.0087 |
| IoU | 0.321 | 0.424 | -0.103 |


> **주의**: Pixel-pooled Accuracy = 0.9356 가 매우 높아 보이는 건 배경 픽셀이 전체의 ~90.5%를 차지하기 때문 (class imbalance). LERF-OVS 평가에서 accuracy는 보조 지표.
...
|---|---|---|---|---|---|---|---|---|---|---|
| A | 24 | 566,788 | 52,588,724 | 6,367,707 | 257,828 | 0.082 | 0.687 | 0.146 | 0.8892 | 0.079 |
| B | 1 | 646 | 2,087,060 | 55,023 | 10,695 | 0.012 | 0.057 | 0.019 | 0.9695 | 0.010 |
| C | 42 | 3,982,516 | 80,872,637 | 2,753,024 | 196,698 | 0.591 | 0.953 | 0.730 | 0.9664 | 0.574 |
...
|---|---|---|---|---|---|---|
| A | 24 | 0.088 | 0.678 | 0.151 | 0.9026 | 0.091 |
| B | 1 | 0.012 | 0.057 | 0.019 | 0.9695 | 0.010 |
| C | 42 | 0.658 | 0.930 | 0.742 | 0.9677 | 0.624 |
...
|---|---|---|---|---|---|
| A1 | 6 | 4.320 | 25.6% | ✅ YES (architecture) | per-SP CLIP feature 생성 단계 수정 (image_encoding.py + merge_proj.py). topk/scoring 미변경. |
| A2 | 12 | 8.810 | 52.1% | ❌ NO (inference setting) | rank in [2,10] = topk 키우면 풀리지만 topk=3은 protocol fix. |
| A3 | 6 | 3.773 | 22.3% | ❌ NO (inference setting) | rank=1인데 topk=3가 distractor 포함 → topk=1 축소가 해결이지만 setting 변경. |
...
전체 67 prompts
├── Type C (성공): 42 (62.7%)
└── Type A (실패): 24 (35.8%) — 손실 16.904 (65.5% of total)
    ├── A1 (rank≥11): 6 (25.0% of A) — 손실 4.320 (25.6% of A)
    ├── A2 (rank 2-10): 12
    └── A3 (rank=1): 6
```

→ **A1 6개 prompt만 해결해도 전체 IoU 손실의 약 17% 회복 가능**.

### 5.3 A1의 평균 rank ≈ 42 — CLIP feature가 정답을 못 알아봄

A1의 6개 prompt에서 정답 SP의 CLIP rank가 평균 **42등** (pool ~745-4136 SPs 중). 즉 위쪽 ~41개 SP가 모두 정답보다 CLIP score 높음.
...
|---|---|---|---|---|---|---|
| figurines | 21 | 0.7942 | 0.3235 | 0.4707 | 9 (43%) | 1 (11% of A) |
| ramen | 14 | 0.7857 | 0.3146 | 0.4711 | 7 (50%) | 2 (29% of A) |
| teatime | 14 | 0.9003 | 0.5680 | 0.3323 | 5 (36%) | 1 (20% of A) |
| waldo_kitchen | 18 | 0.7512 | 0.5300 | 0.2211 | 3 (17%) | 2 (67% of A) |
...
| teatime | bear nose | 3 | 0.974 | 0.094 | 0.880 | A | 5 | 0.097 | 1.000 | 1.00 |
| figurines | pink ice cream | 3 | 0.933 | 0.064 | 0.870 | A | 2 | 0.056 | 0.951 | 1.00 |
| ramen | egg | 7 | 0.917 | 0.093 | 0.824 | A | 2 | 0.092 | 1.000 | 1.00 |
| ramen | kamaboko | 7 | 0.877 | 0.055 | 0.822 | A | 1 | 0.057 | 1.000 | 1.00 |
| teatime | apple | 5 | 0.955 | 0.142 | 0.813 | A | 2 | 0.117 | 0.979 | 1.00 |
| ramen | onion segments | 7 | 0.803 | 0.000 | 0.803 | A | 13 | 0.000 | 0.000 | 0.74 |
| waldo_kitchen | spatula | 1 | 0.903 | 0.133 | 0.770 | A | 2 | 0.135 | 0.939 | 0.99 |
| figurines | red apple | 3 | 0.938 | 0.175 | 0.763 | A | 2 | 0.151 | 0.975 | 1.00 |
| figurines | green apple | 4 | 0.947 | 0.205 | 0.742 | A | 1 | 0.194 | 0.973 | 0.99 |
| teatime | hooves | 2 | 0.718 | 0.000 | 0.718 | A | 57 | 0.000 | 0.000 | 0.00 |
| waldo_kitchen | spoon | 1 | 0.711 | 0.000 | 0.711 | A | 42 | 0.000 | 0.000 | 0.61 |
| ramen | corn | 5 | 0.711 | 0.000 | 0.710 | A | 2 | 0.000 | 0.021 | 0.99 |
| figurines | pirate hat | 4 | 0.899 | 0.189 | 0.710 | A | 2 | 0.180 | 0.959 | 0.99 |
| ramen | wavy noodles | 7 | 0.820 | 0.129 | 0.691 | A | 3 | 0.138 | 0.971 | 1.00 |
...
|---|---|---|---|---|
| LangSplat (per-image → per-scene) | 0.8079 | 0.4340 | 0.3738 | 논문/eval_seg.py 표준 |
| Per-prompt mean across frames → per-scene → overall | 0.8110 | 0.4272 | 0.3838 | failure 분석에 사용 |
| Flat prompt mean (all prompts equal weight) | 0.8091 | 0.4241 | 0.3850 | 참고용 (scene size 무시) |
