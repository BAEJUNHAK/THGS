...
def evaluate(feat_dir, gt_dir, out_csv, arm, level=1,
...
if __name__ == '__main__':
    ...
    p.add_argument('--level', type=int, default=1, help='seg_map level (0..3); default 1 = s')
    p.add_argument('--min_iou', type=float, default=0.2,
                   help='mask 가 GT category 로 라벨되기 위한 최소 IoU')
    ...
    p.add_argument('--include_background', action='store_true',
                   help='IoU < min_iou 인 mask 도 별도 cluster 로 포함')
    args = p.parse_args()

    evaluate(args.feat_dir, args.gt_dir, args.out_csv, args.arm,
             level=args.level,
