"""
Originally from LangSplat, adding CUDA acceleration and some modifications.
"""
import os
import random
import argparse
...
if __name__ == '__main__':
    ...
    data_listi = os.listdir(img_folder)
    data_listi.sort()
    ...
    while True:
        try:
            feat_folder = os.path.join(source_path, 'language_features')
            q = 0
            if os.path.exists(feat_folder):
                q = max(len(os.listdir(feat_folder)) // 2, 0)
                print(q, len(os.listdir(feat_folder)))
            if q >= len(data_listi):
                break
            data_list = data_listi[q:]
            ...
            print(f"resolution: {resolution}, first file{data_list[0]}, last file{data_list[-1]}")

            save_folder = os.path.join(source_path, 'language_features')
